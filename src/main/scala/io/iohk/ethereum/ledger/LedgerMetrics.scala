...
class LedgerMetrics(metrics: Metrics, getBestBlockNumber: () ⇒ Double) extends MetricsContainer {
   ...
   */
  final val ImportBlockNumber = metrics.newGauge("ledger.import.block.number", getBestBlockNumber)

  // Note ... and this is the name we want to migrate to.
  // Note When this happens, the below metric must move to another `MetricsContainer`
  metrics.newGauge("best.block.number.gauge", getBestBlockNumber)

  /**
   * Counter for the imported blocks, since last reboot.
   */
  final val ImportedBlocksCounter = metrics.newCounter("ledger.imported.blocks.counter")

  /**
   * Counter for the imported transactions, since last reboot.
   */
  final val ImportedTransactionsCounter = metrics.newCounter("ledger.imported.transactions.counter")
}
