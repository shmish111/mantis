...
class JsonRpcController(
  ...
  def handleRequest(request: JsonRpcRequest): Future[JsonRpcResponse] = {
    ...
    if (config.disabledMethods.contains(method)) {
      ...
      metrics.MethodsCounter.increment()
      metrics.methodCounter(method).increment() // FIXME DOS attack with random names
      ...
      result.andThen {
        case Success(JsonRpcResponse(_, _, Some(JsonRpcError(code, _, _)), _)) =>
          ...
          metrics.methodErrorCounter(method).increment()

        case Success(JsonRpcResponse(_, _, None, _)) =>
          metrics.MethodsSuccessCounter.increment()
          metrics.methodSuccessCounter(method).increment()
        ...
        case Failure(t) =>
          ...
          metrics.methodExceptionCounter(method).increment()
          log.error("For request " + request, t)
      }
    }
  }
